...
Fixpoint kindOfTyCon0 (tc : tycon0) :=
  match tc with
  | TyConFun      => KFun KData (KFun KData KData)
  | TyConUnit     => KData
  | TyConBool     => KData
  | TyConNat      => KData
  end.
...
Fixpoint kindOfTyCon1 (tc : tycon1) :=
  ...
  | TyConUse      => KFun KRegion KClosure
  end.
...
(********************************************************************)
(* Type constructors with at least three applied arguments. *)
Inductive tycon3 : Type :=
  | TyConComp    : tycon3.
Hint Constructors tycon3.


Fixpoint kindOfTyCon3 (tc : tycon3) :=
  match tc with
  | TyConComp    => KFun KData (KFun KEffect (KFun KClosure KData))
  end.


Fixpoint tycon3_beq tc1 tc2 :=
  match tc1, tc2 with
  | TyConComp,      TyConComp     => true
  end.
